...
Commands.list = {
        ...
        for (var i in gameServer.clients) {
            ...
            if (client.pID == id) {
                ...
                if (client.minionControl === true && isNaN(add)) {
                    ...
                    client.miNum = Math.abs(client.miNum + add);
    ...
    ban: function (gameServer, split) {
        ...
        var fmessage = getName(socket.playerTracker._name);
        if (fmessage.search(name) != -1 ) {
    ...
    r: function (gameServer, split) {
        Commands.list.restart(gameServer, split);
    }
};
...
function ban(gameServer, split, ip) {
    ...
    gameServer.clients.forEach(function (socket) {
        ...
        if (!socket || !socket.isConnected || !gameServer.checkIpBan(ip) || socket.remoteAddress != ip)
            return;
        // remove player cells
        Commands.list.kick(gameServer, split);
        // disconnect
