...
UpdateNodes.prototype.writeUpdateItems6 = function(writer) {
    ...
    for (const node of this.updNodes) {
        ...
        //if (node.name == 'pellet' || node.name == 'ejectMass' || node.name == 'coin' || node.name == 'portal' || node.name == 'virus') {
        //    if (this.playerTracker.tickNodes <= maxTick) continue;
        //}
        ...
        if (this.playerTracker.clientV != '2.01.4') writer.writeStringZeroUtf8(node.name); // type
    ...
    writer.writeUInt32(0);  // Cell Update record terminator
    /*
    if (this.playerTracker.tickNodes > maxTick) {
        this.playerTracker.tickNodes = 0;
    }

    this.playerTracker.tickNodes++;*/
};
